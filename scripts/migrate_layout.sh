...
mkdir -p "$SERVICES/mysql" "$SERVICES/redis" \
    "$DATA/ssl" "$DATA/log/system" "$DATA/mysql/db" "$DATA/redis" \
...
if [ -f "$SERVICES/mysql/mysql_utils.sh" ]; then
    # shellcheck source=/dev/null
    source "$SERVICES/mysql/mysql_utils.sh"
fi
if [ -f "$SERVICES/redis/redis_utils.sh" ]; then
    # shellcheck source=/dev/null
    source "$SERVICES/redis/redis_utils.sh"
fi

preserve_mysql_pass() {
    local dest="${HIDDIFY_DATA:-$DATA}/mysql/mysql_pass"
    local src
...
preserve_redis_pass() {
    local dest_pass="${HIDDIFY_DATA:-$DATA}/redis/redis_pass"
    local src pass=""
    ...
    for src in \
        "${HIDDIFY_DATA:-$DATA}/redis/redis.conf" \
