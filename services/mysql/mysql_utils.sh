...
function hiddify_mysql_pass_file() {
    echo "$HIDDIFY_DATA/mysql/mysql_pass"
}

function hiddify_mysql_datadir() {
    echo "$HIDDIFY_DATA/mysql/db"
}

function hiddify_mysql_dropin() {
    echo "/etc/mysql/mariadb.conf.d/99-hiddify.cnf"
}

function ensure_mysql_data_dirs() {
    # Parent only — do not pre-create empty db/ (blocks atomic mv during migrate)
    mkdir -p "$HIDDIFY_DATA/mysql"
}
...
function ensure_mysql_password() {
    ...
    local pass_file legacy
    pass_file="$(hiddify_mysql_pass_file)"
    legacy="$HIDDIFY_SERVICES/mysql/mysql_pass"
    mkdir -p "$(dirname "$pass_file")"
    HIDDIFY_MYSQL_PASS_IS_NEW=0

    if [ ! -f "$pass_file" ] && [ -f "$legacy" ]; then
        mv "$legacy" "$pass_file"
        chmod 600 "$pass_file"
    elif [ -f "$legacy" ]; then
        rm -f "$legacy"
    ...
    if [ ! -f "$pass_file" ]; then
        ...
        hiddify_random_password >"$pass_file"
        chmod 600 "$pass_file"
...
function configure_mysql_server() {
    local datadir dropin conf
    datadir="$(hiddify_mysql_datadir)"
    dropin="$(hiddify_mysql_dropin)"
